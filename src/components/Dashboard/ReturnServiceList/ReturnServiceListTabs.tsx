...
import { ScholarProfileDialog } from "@/components/Users/scholar-profile-dialog"
...
function ScholarTable({ data }: { data: Scholar[] }) {
    ...
    const table = useReactTable({
      ...
      initialState: {
        pagination: {
          pageSize: 3, // Increased page size for better use of space
        },
      },
    })
    ...
    return (
      <div className="flex h-full w-full flex-col space-y-4">
        ...
        <div className={`rounded-md border flex-1 overflow-hidden transition-all duration-200 ease-in-out ${
    sidebarState === 'collapsed' ? 'w-[50rem]' : 'w-[42rem] overflow-auto'
  }`}>
          <Table>
  ...
  export default function RSScholarListTabsComponent({ scholars }: ScholarProps) {
    const { state: sidebarState } = useSidebar()
      ...
      <div className={`h-full flex flex-col transition-all duration-200 ease-in-out`}>
        <Tabs defaultValue="all" className="h-full flex flex-col" onValueChange={setActiveTab}>
          <TabsList className="grid w-full grid-cols-3 mb-4">
            <TabsTrigger value="all" className="data-[state=active]:bg-ys data-[state=active]:text-white">
              All ({statusCounts.all})
            </TabsTrigger>
            <TabsTrigger value="complete" className="data-[state=active]:bg-ys data-[state=active]:text-white">
              Complete ({statusCounts.complete})
            </TabsTrigger>
            <TabsTrigger value="incomplete" className="data-[state=active]:bg-ys data-[state=active]:text-white">
          ...
          <TabsContent value="complete" className="flex-1 mt-0">
            <ScholarTable data={filteredData} />
          </TabsContent>
