...
export function EventSection({ title, events, isLoading, onEventSelect }: EventSectionProps) {
  ...
  const handleViewAll = () => {
    ...
    router.push(
      `/events/cso/viewevents?title=${encodeURIComponent(title)}&status=${encodeURIComponent(events[0].status)}`,
    )
  }
                  ...
                  <CardContent className="p-3">
                    ...
                    <div className="flex justify-between">
                      <Button
                        size="sm"
                        variant="secondary"
                        className={`text-xs bg-[#191851] text-white hover:bg-blue-800 ${title === "Ongoing Events" ? "w-full" : title === "Upcoming Events" ? "" : "w-full"}`}
