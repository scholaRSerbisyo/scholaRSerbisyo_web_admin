...
export function SchoolEventSection({
  schoolname,
  admintype,
  title,
  events,
  ...
  const handleViewAll = () => {
    router.push(
      `/events/school/viewevents?type=${encodeURIComponent(schoolname)}&title=${encodeURIComponent(title)}&status=${encodeURIComponent(events[0]?.status || "")}`,
    )
  }
